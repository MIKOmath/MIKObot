...
def start_class(seminar_id, base_url, token):
    """
    Updates the 'featured' field of a specific seminar to True using the API.

    Args:
        seminar_id (int): ID of the seminar to update.
        base_url (str): The base URL of the API (e.g., 'https://mikomath.org/api/').
        token (str): The API token for authentication.

    Returns:
        dict: Response data or error message from the API.
    """
    endpoint = f"{base_url}/seminars/{seminar_id}/"
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"Seminar {seminar_id} marked as featured successfully.")
